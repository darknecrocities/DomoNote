    ...
    <div className="flex-1 flex flex-col h-full bg-black p-8 overflow-y-auto max-w-6xl mx-auto w-full select-none font-sans">
      ...
      <div className="border-b border-zinc-850 pb-5 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-white tracking-tight">Automated Schedule</h1>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between text-xs font-mono text-zinc-500 uppercase tracking-wider pb-2 border-b border-zinc-850">
            <span>Today's Time Blocks</span>
            <span>{events.filter((e) => e.completed).length} / {events.length} Done</span>
          </div>

          <div className="space-y-3">
            {events.map((ev) => {
              const categoryBadge = {
                meeting: 'bg-zinc-800 text-zinc-200 border-zinc-700',
                'deep-work': 'bg-white text-black font-bold border-zinc-300',
                review: 'bg-zinc-900 text-zinc-400 border-zinc-800',
                manual: 'bg-zinc-850 text-zinc-300 border-zinc-750',
              }[ev.category];
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs text-zinc-400">{ev.time}</span>
                      ...
                      <h4
                        className={`text-sm font-semibold tracking-tight mt-1 truncate ${
                          ev.completed ? 'line-through text-zinc-600' : 'text-zinc-100'
                        }`}
                      >
                        {ev.title}
                      </h4>
                      {ev.notes && (
                        <p className="text-xs text-zinc-500 mt-0.5 truncate">{ev.notes}</p>
                      )}
                    </div>
                  </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <form
            ...
            className="p-5 rounded-2xl border border-zinc-800 bg-zinc-950 space-y-4 shadow-xl"
          >
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 text-xs text-zinc-200 placeholder-zinc-600 outline-none focus:border-zinc-700"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-2.5 py-1.5 rounded-lg bg-zinc-900 border border-zinc-800 text-xs text-zinc-200 outline-none focus:border-zinc-700 font-mono"
                />
              </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-2.5 py-1.5 rounded-lg bg-zinc-900 border border-zinc-800 text-xs text-zinc-200 outline-none focus:border-zinc-700"
              >
          ...
          <div className="p-5 rounded-2xl border border-zinc-850 bg-zinc-950/60 font-mono text-xs space-y-2 text-zinc-400">
            <div className="flex items-center gap-2 text-zinc-200 font-bold text-xs mb-1">
