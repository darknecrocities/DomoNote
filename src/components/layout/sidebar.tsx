        ...
        {collapsed ? (
          <div className="flex flex-col items-center gap-2 py-2">
            <span
              ...
              title={isConnected ? `Ollama Active (${selectedModel || 'Ready'})` : 'Ollama Offline — Click to Start'}
            />
            {!isConnected ? (
              <button
                onClick={() => startOllamaService()}
                disabled={isChecking}
                className="p-1.5 rounded-md bg-slate-900 dark:bg-zinc-800 text-emerald-400 hover:text-emerald-300 transition-colors"
                title="Start Local AI"
              >
                {isChecking ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Power className="w-3.5 h-3.5" />}
              </button>
            ) : (
              <button
          ...
          <div className="p-2.5 rounded-lg bg-white dark:bg-zinc-900/90 border border-slate-200 dark:border-white/10 shadow-sm">
            ...
            {!isConnected && (
              <button
                onClick={() => startOllamaService()}
                disabled={isChecking}
                className="w-full flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-md bg-slate-900 hover:bg-black dark:bg-zinc-800 dark:hover:bg-zinc-700 text-white text-[11px] font-semibold transition-all shadow-sm active:scale-95 disabled:opacity-75 disabled:cursor-not-allowed whitespace-nowrap"
                title="Launch Local AI or open guided recovery commands"
              >
                {isChecking ? (
                  <>
